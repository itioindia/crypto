...
// Function for Display Listing and wallet list with balance
...
func AdminMembersView(c *fiber.Ctx) error {
	...
	//database.DB.Db.Table("client_master").Order("client_id desc").Limit(limit).Offset(offset).Find(&memberList)
